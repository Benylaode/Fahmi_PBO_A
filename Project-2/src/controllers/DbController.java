...
public class DbController extends MyConfig {
    ...
    public static void getDatabase(JTable tabel_data,JTextField lbnama,JTextField lbharga ,JTextField lbjumlah ) {
        connection();
        try {
            ...
            DefaultTableModel mode = (DefaultTableModel) tabel_data.getModel();
            mode.setRowCount(0);
            while(resultSet.next()) {
                String hrg = Integer.toString(resultSet.getInt("harga"));
                String jml = Integer.toString(resultSet.getInt("jumlah"));
                lbnama.setText(resultSet.getString("nama"));
                lbharga.setText(hrg);
                lbjumlah.setText(jml);
    ...
    public static Produk ProdukByNama(String nama) {
       Produk produk = null;
        connection();
        ...
        try {
            ...
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                Produk product = new Produk(resultSet.getInt("id"), resultSet.getString("nama"), resultSet.getLong("harga"), resultSet.getInt("jumlah"));
                 produk = product;
            }
    ...
    public static boolean Masukkan(String nama, long harga, int jumlah) {
        connection();
